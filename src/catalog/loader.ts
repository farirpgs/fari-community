...
export const loader = {
  async getCreatorData(creatorSlug: string): Promise<ICreator> {
    ...
    const files = await fs.readdir(
      path.join(process.cwd(), "src", "catalog", "creators"),
      { withFileTypes: true }
    );
  ...
  async getCreatorProjects(creatorSlug: string): Promise<Array<IProject>> {
    const fs = await import("fs/promises");
    const files = await fs.readdir(
      path.join(process.cwd(), "src", "catalog", "creators", creatorSlug),
      { withFileTypes: true }
    );
    const folders = files.filter((f) => f.isDirectory());
    const result = await Promise.all(
  ...
  async getProjectData(
    ...
    const files = await fs.readdir(
      path.join(process.cwd(), "src", "catalog", "creators", creatorSlug, slug),
      { withFileTypes: true }
    );
